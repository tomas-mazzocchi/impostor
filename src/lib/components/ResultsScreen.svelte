...
<div class="flex flex-col gap-8 items-center">
	<h1 class="text-center text-4xl">¡Fin del Juego!</h1>
	...
	<div class="p-8 rounded-2xl text-center max-w-[500px] text-white {gameState.winner === 'impostor' ? 'bg-danger' : 'bg-success'}">
		{#if gameState.winner === 'impostor'}
			<h2 class="text-3xl mb-2 m-0">¡Ganó el Impostor!</h2>
			<p class="text-xl m-0">El impostor evitó ser detectado exitosamente.</p>
		{:else}
			<h2 class="text-3xl mb-2 m-0">¡Ganaron los Jugadores!</h2>
			<p class="text-xl m-0">¡El impostor fue atrapado!</p>
		{/if}
	</div>

	<div class="text-center">
		<h2 class="text-2xl mb-4">El Impostor era:</h2>
	...
	<div class="bg-gray-light p-6 rounded-lg max-w-[600px] w-full">
		<h2 class="text-xl mb-4">Todas las Palabras</h2>
		<div class="flex flex-col gap-2 mt-4">
			{#each gameState.players as player}
				<div class="bg-white p-3 rounded">
					<strong>{player.name}:</strong>
					{#if player.role === 'impostor'}
						<span class="text-danger italic">??? (Impostor - no sabía la palabra)</span>
					{:else}
